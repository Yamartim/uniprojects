...
void cenaFala(int est, int resp, char nome[10])
{
    ...
    printf("<a href='http://cap.dc.ufscar.br/~726395/cgi-bin/dialogo.cgi?lvl=%d&resp=-1&nome=%s'> Próximo>> </a>", est+1, nome );
...
// Links com as opções de respostas (^-^ )
    if(rand()%2)//a ordem da impressao das opcoes eh definida aleatoriamente
    {
        ...
        //printf("\n html.... %s", frase);
		printf("<a href='http://cap.dc.ufscar.br/~726395/cgi-bin/dialogo.cgi?lvl=%d&resp=0&nome=%s'> <div class='dialogo'>%s</div> </a>",est+1,nome,frase);
		
        buscafrase(frase, op2, est);
        //printf("\n html.... %s\n\n", frase);
		printf("<a href='http://cap.dc.ufscar.br/~726395/cgi-bin/dialogo.cgi?lvl=%d&resp=1&nome=%s'> <div class='dialogo'>%s</div> </a>",est+1,nome,frase);
    }
    else
    {
        buscafrase(frase, op2, est);
